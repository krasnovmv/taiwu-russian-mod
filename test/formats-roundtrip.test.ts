...
import { listDlcEventFiles, listEventFiles, listSourceFiles } from "../src/scan.js";
...
// Always cover the full event corpus here (root + DLC), even though the root
// folder is gated out of the pipeline by default (TAIWU_EVENTS) — the round-trip
// safety net should not depend on whether quest translation is currently on.
const files = [
  ...new Set([
    ...(await listSourceFiles()),
    ...(await listEventFiles()),
    ...(await listDlcEventFiles()),
  ]),
];
